...
\author{Dylan~R.~Sutton\altaffilmark{\Melbourne}, Chang~Feng\altaffilmark{\uci}, and Christian~L.~Reichardt\altaffilmark{\Melbourne}}
...
%\changed{Most of the improvement is due to the addition of the \bicepkeck{} and \sptpol{} data.} 
...
\keywords{ cosmic background radiation --- early universe --- magnetic fields --- polarization }
...
The parameter $\beta$ relates the timing of neutrino decoupling and the generation of the PMF, and is defined by $\beta = {\rm ln}(a_{\nu}/a_{\rm{PMF}})$, where $a_x$ represents the scale factor at the respective events. }
...
If we follow \citet{polarbear15} who used $\beta \in [0,39]$, the \planck-only upper limit is nearly eight times higher, and the combined dataset limit nearly two times higher. }
...
Quantum mechanical zero-point fluctuations during inflation can generate an approximately scale-invariant PMF with a spectral index close to $n_B = -3$ \citep[e.g.,][]{turner88, mack01,2008PhRvD..78f3012K}. %\footnote{We select $n_B = -2.9$ as opposed to another number near -3 to facilitate comparison to other PMF papers, while avoiding the singularity at -3.}
...
\removed{
We have compared the calculated templates to the outputs of the earlier CMBACT-derived code\footnote{\url{http://www.sfu.ca/~levon/cmbact.html}}
\citep{pogosian99}.}
...
\st{We have chosen to use the more recent} \citet{zucca16} \st{code instead of the CMBACT-derived code, largely due to our desire to extend the calculation out to $\ell=10000$ for the Fisher matrix forecasts.}
\textcolor{red}{We use the code from} \citet{zucca16} \textcolor{red}{which allows us to extend the calculation out to $\ell=10000$ for the Fisher matrix forecasts.}
...
Adding either one to \planck{} yields the same upper limit: $\apmf^{\planck+\bicepkeck} <  \upperAbicep$ versus  $\apmf^{\planck+\sptpol}) < \upperAsptpol$. 
...
The other datasets do not have a large impact on the observed limits for a scale-invariant PMF, although they would for bluer PMFs. 
...
\changed{It is possible the difference is due to poor sampling of low values of \bpmf{} when importance sampling the chain. }
\st{The improvement is due to a difference in the calculated tensor PMF power (for the same parameters) between the CMBACT-derived and} \citet{zucca16} \st{codes.}
\st{Using the CMBACT-derived code that} \citet{planck15-19} \st{used, we find a consistent upper limit of $\bpmf < 1.9$\,nG.}
\changed{Adding the ground-based polarization measurements reduces the 95\% CL upper limit only slightly to $\bpmf < \upperBall$\,nG. }
...
Adding  EE and BB bandpowers from two stage-III experiments, \sptnew{} and \simons{}, reduces the uncertainty by an order of magnitude in the \changed{8}-parameter model to $\sigma(\apmf)=0.015$. 
...
Thus within the \changed{12}-parameter model,  the addition of stage III CMB experiments improves the \apmf{} uncertainty by a factor of $\sim$\,35 to  $\sigma(\apmf)=0.022$. 
...
We have shown that an ideal version of CMB-S4 might decrease the 95\% CL upper limits eight-fold, to $\apmf < 0.0025$, and less optimistic versions can still set upper limits on order of $\apmf < 0.006$ in a \changed{12}-parameter cosmological model with  \lcdm{} +  $r$ + \nrun{} +  \neff{} + \mnu{} + \apmf{} \changed{+$\beta$}. 

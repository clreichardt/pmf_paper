...
\changed{The power in the tensor mode depends on the timing of PMF generation relative to neutrino decoupling. 
...
We use the code developed by \citet{zucca16} to compute the tensor power spectra with a fixed magnetic field strength but different $\beta$ values. Given the fact that the amplitude of the tensor mode reaches the maximum around $\ell$ = 100, we assume a simple power law and extrapolate the scaling relation between the power and the parameter $\beta$ from an ensemble of amplitudes at $\ell$ =100. Similar to the vector mode, the tensor power depends on the amplitude of the magnetic field as well. 
%We use this $\beta$ power law form to handle the PMF timing dependence. 
Thus the tensor PMF power scales as:
\be \label{eqn:betascaling}
C_{\ell}^{\rm{tens}} \propto  \bpmf^4 \beta^{1.9},
\ee
which corresponds to the last term in Eq. (\ref{clmodel}).
}
...
\changed{similar to} the limit found by \citet{planck15-19} of $\bpmf < 2.0$\,nG with $n_B=-2.9$ held fixed as we have done. 
...
\changed{Adding the ground-based polarization measurements significantly reduces the 95\% CL upper limit to $\bpmf < \upperBall$\,nG. }
\st{The improvement from adding more data is less than one might predict because the combined likelihood peaks approximately $1\,\sigma$ above zero, whereas the \planck{}-only constraints peaks at zero. }
\changed{Due to the inclusion of the \bicepkeck{} and POLARBEAR data, these results are slightly lower than the \planck{} + \sptpol{} upper limit of $<1.2\,$nG quoted by \citet{zucca16}.}

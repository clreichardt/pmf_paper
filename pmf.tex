...
If this is indeed the case then the GeV results set lower limits on the intergalactic magnetic field strength of $10^{-9} - 10^{-6}$\,nG \citep{tavecchio10,taylor11,dermer11,vovk12}. There are, however, alternative explanations involving plasma instabilities which may result in positron-electron pairs instead depositing their energy into the IGM as heat \citep{broderick12}.
...
The PMF anisotropy is assumed to be drawn from a Gaussian distribution with a power law power spectrum, $A_B k^{n_B}$, with $n_B$ the spectral index and $A_B$ the PMF power normalization. 
...
We have compared calculated templates to the outputs of the earlier CMBACT code\footnote{\url{http://www.sfu.ca/~levon/cmbact.html}} \citep{pogosian99}. 
...
The decision to use the  \citet{zucca16} code instead of CMBACT is based on the need to extend the calculation out to $\ell=10000$ for the Fisher matrix forecasts. 
...
The difference is due to a difference in the calculated tensor PMF power (for the same parameters) between the CMBACT and \citet{zucca16} codes. 
Using the CMBACT code that \citet{planck15-19} used, we find a completely consistent upper limit of $\bpmf < 1.9$\,nG. 
...
Between each stage of ground-based CMB experiment, the number of detectors increases by an order of magnitude. Adding detectors will always reduce the noise in the instrument, making it easier to isolate the PMF signal, thereby reducing the uncertainty on PMF measurements. When quantifying the effects of the number of detectors it is more informative to use polarisation noise, as detector count relies on knowing the individual sensitivity of each detector. As such, we give our result in terms of polarisation noise, noting that polarisation noise scales with detector count as $\sqrt{N_{detectors}}$

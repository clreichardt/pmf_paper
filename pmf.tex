...
With the minimal cosmological model, the 1-sigma forecasts for \planck{}+\ho{} is $\sigma(\apmf)=\fisherAplk$ for a nearly-scale invariant PMF spectrum (i.e.~$n_b=-2.9$). (\tbd{need to check how this compares}This is consistent with, although slightly worse than, the upper limits from the real data).  
...
\includegraphics[width=0.45\textwidth,clip,trim={1.5cm 12.5cm 11cm 7.5cm}]{pmf_knee.pdf}
  ...
  The forecasted uncertainty on the PMF power, \apmf{}, falls as expected as the low-frequency map noise decreases for a nearly scale-invariant PMF ($n_b = -2.9$). 
  In particular, achieving a map 1/f knee frequency at or below $\ell = 200$ substantially improves the potential power of CMB-S4 PMF searches for nearly scale-invariant PMFs. 
  The black solid line shows the forecast uncertainty as a function of area in the 11-parameter cosmological model with $n_B = -2.9$. 
  ...
  As we expect, reducing low-frequency noise would lead to better PMF constraints. 
      Note however, the dependence on the low-frequency noise quickly disappears as $n_b$ increases (not shown); the improvement across this range of knees is small for $n_B=-1$ and negligible for $n_B=2$. 

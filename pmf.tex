...
\author{C.~L.~Reichardt\altaffilmark{\Melbourne}, D.~Sutton\altaffilmark{\Melbourne}, and C.~Feng\altaffilmark{\uci}}
...
For vector modes, we can translate the amplitude constraint on this template to a constraint rms magnetic field strength on 1\,Mpc scales , \bpmf, using the expected scaling:
\be
\apmf \propto \bpmf^4.
\ee
Note that the situation is more complicated for the tensor PMF modes where this power law scaling has not been demonstrated. 
An unfortunate consequence of this steep scaling is that substantial reductions in the observationally allowed vector-mode power lead to only modestly better limits on the magnetic field strength. 
